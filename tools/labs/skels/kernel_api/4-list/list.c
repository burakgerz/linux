...
static void task_info_purge_list(void)
{
	...
	list_for_each_safe (pos, next, &head) {
		// use list_entry (wrapper for container_of)

		//ti = container_of(next, struct task_info, list); // BUG!
		// next is only for internal use of list_for_each_safe, it will save pos->next in there.
		// Important when you delete pos pointer, because list_for_each() is doing pos->next.
		// When pos deleted and you do pos->next: Undefined behaviour!

		ti = container_of(pos, struct task_info, list);
		list_del(pos);
		kfree(ti);
		task_info_print_list("loop...");
	}
}
