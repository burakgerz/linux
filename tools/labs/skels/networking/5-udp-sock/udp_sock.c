...
#define print_sock_address(addr)                                               \
	do {                                                                   \
		printk(LOG_LEVEL "connection established to " NIPQUAD_FMT      \
				 ":%d\n",                                      \
...
static int my_udp_msgsend(struct socket *s)
{
	 ...
	 * error code.
	 */
	err = kernel_sendmsg(s, &msg, (struct kvec *)&iov, 1, len);
	if (err < 0) {
		pr_info("Error: kernel_sendmsg()");
		return err;
	}
	return 0;
}
